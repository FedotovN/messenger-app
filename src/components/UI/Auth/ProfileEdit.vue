<template>
  <form class="relative flex flex-col sm:gap-4 gap-2 min-w-[220px] sm:max-w-[480px] sm:h-auto w-full h-full rounded px-4 py-2 bg-white dark:bg-dark-200" @submit.prevent="onSubmit">
    ...
    <main class="sm:overflow-visible flex flex-col flex-1 overflow-y-scroll scrollbar-hide sm:gap-4 gap-2">
        <div class="flex sm:flex-row flex-col gap-6 items-center flex-wrap w-full">
            <div class="sm:w-[6rem] w-[12rem] flex flex-col gap-2 text-center items-center mt-6">
                ...
                class="flex justify-center items-center w-40 h-40 sm:w-[6rem] sm:h-[6rem] rounded-full bg-gray-200 cursor-pointer transition-all hover:opacity-75 overflow-hidden" title="Загрузка фото">
            ...
            <div class="min-w-[2rem] sm:flex sm:flex-col flex-1 justify-center w-full grid grid-cols-10 gap-2">
                ...
                <base-input :value="user.email" :label="$t('form.placeholders.email')" disabled class="col-span-6"></base-input>
            </div>
        </div>
        <base-textarea
        class='max-h-full'
